...
def transform_and_process_data(df):
    ...
    initial_transformer = joblib.load('D:\ALL_MY_PROJECT\ML_preformance_sportive_plateforme/pipeline/custom_attribute_object.pkl')
    df = initial_transformer.transform(df)    

    transformer = joblib.load('D:\ALL_MY_PROJECT\ML_preformance_sportive_plateforme/pipeline/full_transformer.pkl')
    transformed_data = transformer.transform(df)

    model = joblib.load('D:\ALL_MY_PROJECT\ML_preformance_sportive_plateforme/models/best_model.pkl')
    predicted_value = model.predict(transformed_data)

    # print('Predicted Value : ', predicted_value)
    
    target_transformer = joblib.load('D:\ALL_MY_PROJECT\ML_preformance_sportive_plateforme/pipeline/target_pipeline.pkl')
